/** PuppetPals Server Class
 * Express + Socket.io server for real-time puppet control
 * This file exports the server class without auto-starting it.
 */
...
class PuppetPalsServer {
  ...
  /**
   * Initialize the server (middleware, routes, socket.io)
   */
  init() {
    this.initMiddleware();
    this.initRoutes();
   ...
   * @returns {Promise<PuppetPalsServer>}
   */
  start(port = config.port, host = config.host) {
    ...
    return new Promise((resolve, reject) => {
      this.server.listen(port, host, () => {
        console.log(`[PuppetPals] Server running on http://${host}:${port}`);
...
export default PuppetPalsServer;